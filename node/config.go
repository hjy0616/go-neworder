...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gnod is "gnod". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gnod", keep that.
	if name == "gnod" || name == "gnod-testnet" {
		name = "Gnod"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gnod" instances.
var isOldGnodResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gnod 1.4 are used if they exist.
	if warn, isOld := isOldGnodResource[path]; isOld {
		oldpath := ""
		if c.name() == "gnod" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGnodResourceWarning, "Using deprecated resource file %s, please move this file to the 'gnod' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func getKeyStoreDir(conf *Config) (string, bool, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = os.MkdirTemp("", "go-neworder-keystore")
		isEphemeral = true
	}
