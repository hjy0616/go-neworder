...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	var (
		req          *SignDataRequest
		useNeworderV = true // Default to use V = 27 or 28, the legacy Neworder format
	)
	mediaType, _, err := mime.ParseMediaType(contentType)
	if err != nil {
		return nil, useNeworderV, err
	}
	...
	case apitypes.ApplicationClique.Mime:
		// Clique is the Neworder PoA standard
		stringData, ok := data.(string)
		if !ok {
			return nil, useNeworderV, fmt.Errorf("input for %v must be an hex-encoded string", apitypes.ApplicationClique.Mime)
		}
	...
	default: // also case TextPlain.Mime:
		// Calculates an Neworder ECDSA signature for:
		// hash = keccak256("\x19Neworder Signed Message:\n${message length}${message}")
		// We expect it to be a string
		if stringData, ok := data.(string); !ok {
			return nil, useNeworderV, fmt.Errorf("input for text/plain must be an hex-encoded string")
		} else {
