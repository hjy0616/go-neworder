...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte Neworder address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an Neworder account at a particular block.
    type Account {
    ...
    # Transaction is an Neworder transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an Neworder account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an Neworder account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an Neworder block by number or by hash. If neither is
