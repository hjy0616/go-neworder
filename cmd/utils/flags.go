...
// Package utils contains internal helper functions for go-neworder commands.
package utils
...
import (
	...
	"github.com/neworder-network/go-neworder/common/fdlimit"
	...
	"github.com/neworder-network/go-neworder/eth"
	ethcatalyst "github.com/neworder-network/go-neworder/eth/catalyst"
	"github.com/neworder-network/go-neworder/eth/downloader"
	"github.com/neworder-network/go-neworder/eth/ethconfig"
	"github.com/neworder-network/go-neworder/eth/filters"
	"github.com/neworder-network/go-neworder/eth/gasprice"
	"github.com/neworder-network/go-neworder/eth/tracers"
	"github.com/neworder-network/go-neworder/ethdb"
	"github.com/neworder-network/go-neworder/ethdb/remotedb"
	"github.com/neworder-network/go-neworder/ethstats"
	"github.com/neworder-network/go-neworder/graphql"
	"github.com/neworder-network/go-neworder/internal/ethapi"
	"github.com/neworder-network/go-neworder/internal/flags"
	"github.com/neworder-network/go-neworder/les"
	lescatalyst "github.com/neworder-network/go-neworder/les/catalyst"
	"github.com/neworder-network/go-neworder/log"
	"github.com/neworder-network/go-neworder/metrics"
	"github.com/neworder-network/go-neworder/metrics/exp"
	"github.com/neworder-network/go-neworder/metrics/influxdb"
	...
	"github.com/neworder-network/go-neworder/p2p/netutil"
...
var (
	...
	RPCGlobalTxFeeCapFlag = &cli.Float64Flag{
		Name:     "rpc.txfeecap",
		Usage:    "Sets a cap on transaction fee (in nod) that can be sent via the RPC APIs (0 = no cap)",
	...
	IgnoreLegacyReceiptsFlag = &cli.BoolFlag{
		...
		Usage:    "Gnod will start up even if there are legacy receipts in freezer",
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.String(DataDirFlag.Name); path != "" {
		if ctx.Bool(RopstenFlag.Name) {
			// Maintain compatibility with older Gnod configurations storing the
...
// for Gnod and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `gnod account list`)")
	log.Warn("-------------------------------------------------------------------")
...
// RegisterEthService adds an Neworder client to the stack.
// The second return value is the full node instance, which may be nil if the
// node is running as a light client.
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *eth.Neworder) {
	if cfg.SyncMode == downloader.LightSync {
		backend, err := les.New(stack, cfg)
		if err != nil {
			Fatalf("Failed to register the Neworder service: %v", err)
		}
...
// RegisterEthStatsService configures the Neworder Stats daemon and adds it to the node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the Neworder Stats service: %v", err)
	}
}
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "gnod.", tagsMap)
		} else if enableExportV2 {
			tagsMap := SplitTagsFlag(ctx.String(MetricsInfluxDBTagsFlag.Name))

			log.Info("Enabling metrics export to InfluxDB (v2)")

			go influxdb.InfluxDBV2WithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, token, bucket, organization, "gnod.", tagsMap)
		}
