// Copyright 2017 The go-neworder Authors
...
// faucet is an Nod faucet backed by a light client.
package main
...
import (
	...
	"github.com/neworder-network/go-neworder/cmd/utils"
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Neworder protocol")
	statsFlag   = flag.String("ethstats", "", "Ethstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Nods to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Neworder and the faucet")
...
var (
	nod = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Nods", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Neworder address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Neworder light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Neworder protocol stack
	client *ethclient.Client   // Client connection to the Neworder chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*enode.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Neworder light client protocol
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to register the Neworder service: %w", err)
	}
...
// close terminates the Neworder connection and tears down the faucet.
...
// apiHandler handles requests for Nod grants and transaction statuses.
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err = send(wsconn, map[string]interface{}{
		"funds":    new(big.Int).Div(balance, nod),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the Neworder address to fund, the requesting user and a profile picture
		var (
			id       string
			username string
			avatar   string
		...
		default:
			//lint:ignore ST1005 This error is to be displayed in the browser
			err = errors.New("Something funky happened, please open an issue at https://github.com/neworder-network/go-neworder/issues")
		}
		if err != nil {
			if err = sendError(wsconn, err); err != nil {
		...
		if timeout = f.timeouts[id]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), nod)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance := new(big.Int).Div(f.balance, nod)
			peers := f.stack.Server().PeerCount()
...
// the uniqueness identifier (user id/username), username, avatar URL and Neworder address to fund on success.
func authTwitter(url string, tokenV1, tokenV2 string) (string, string, string, common.Address, error) {
	...
	// and scrape it for the Neworder address and profile URL. We need to load
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", "", common.Address{}, errors.New("No Neworder address found to fund")
	}
	var avatar string
...
// API, returning the user id, username, avatar URL and Neworder address to fund on
// success.
...
// returning the username, avatar URL and Neworder address to fund on success.
func authFacebook(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Neworder address and profile URL.
	//
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", common.Address{}, errors.New("No Neworder address found to fund. Please check the post URL and verify that it can be viewed publicly.")
	}
	var avatar string
...
// authNoAuth tries to interpret a faucet request as a plain Neworder address,
