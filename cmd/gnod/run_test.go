...
import (
	...
	"github.com/neworder-network/go-neworder/internal/cmdtest"
	"github.com/neworder-network/go-neworder/rpc"
)

type testgnod struct {
	*cmdtest.TestCmd
...
func init() {
	// Run the app if we've been exec'd as "gnod-test" in runGnod.
	reexec.Register("gnod-test", func() {
...
// spawns gnod with the given command line args. If the args don't set --datadir, the
// child g gets a temporary data directory.
func runGnod(t *testing.T, args ...string) *testgnod {
	tt := &testgnod{}
	...
	// Boot "gnod". This actually runs the test binary but the TestMain
	// function will prevent any tests from running.
	tt.Run("gnod-test", args...)
