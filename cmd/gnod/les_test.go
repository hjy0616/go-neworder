...
func (g *gnodrpc) killAndWait() {
	g.gnod.Kill()
	g.gnod.WaitExit()
}

func (g *gnodrpc) callRPC(result interface{}, method string, args ...interface{}) {
	if err := g.rpc.Call(&result, method, args...); err != nil {
		g.gnod.Fatalf("callRPC %v: %v", method, err)
	}
}

func (g *gnodrpc) addPeer(peer *gnodrpc) {
	g.gnod.Logf("%v.addPeer(%v)", g.name, peer.name)
	enode := peer.getNodeInfo().Enode
	peerCh := make(chan *p2p.PeerEvent)
	sub, err := g.rpc.Subscribe(context.Background(), "admin", peerCh, "peerEvents")
	if err != nil {
		g.gnod.Fatalf("subscribe %v: %v", g.name, err)
	}
	...
	select {
	case ev := <-peerCh:
		g.gnod.Logf("%v received event: type=%v, peer=%v", g.name, ev.Type, ev.Peer)
	case err := <-sub.Err():
		g.gnod.Fatalf("%v sub error: %v", g.name, err)
	case <-timeout:
		g.gnod.Error("timeout adding peer after", dur)
	}
}

// Use this function instead of `g.nodeInfo` directly
func (g *gnodrpc) getNodeInfo() *p2p.NodeInfo {
	if g.nodeInfo != nil {
		return g.nodeInfo
	}
...
func startGnodWithIpc(t *testing.T, name string, args ...string) *gnodrpc {
	ipcName := fmt.Sprintf("gnod-%d.ipc", atomic.AddUint32(&nextIPC, 1))
	...
	ipcpath := ipcEndpoint(ipcName, g.gnod.Datadir)
	// We can't know exactly how long gnod will take to start, so we try 10
...
func initGnod(t *testing.T) string {
	args := []string{"--networkid=42", "init", "./testdata/clique.json"}
	t.Logf("Initializing gnod: %v ", args)
	g := runGnod(t, args...)
	datadir := g.Datadir
	g.WaitExit()
	return datadir
...
func startLightServer(t *testing.T) *gnodrpc {
	datadir := initGnod(t)
	t.Logf("Importing keys to gnod")
	runGnod(t, "account", "import", "--datadir", datadir, "--password", "./testdata/password.txt", "--lightkdf", "./testdata/key.prv").WaitExit()
	account := "0x02f0d131f1f97aef08aec6e3291b957d9efe7105"
	server := startGnodWithIpc(t, "lightserver", "--allow-insecure-unlock", "--datadir", datadir, "--password", "./testdata/password.txt", "--unlock", account, "--mine", "--light.serve=100", "--light.maxpeers=1", "--nodiscover", "--nat=extip:127.0.0.1", "--verbosity=4")
	return server
}

func startClient(t *testing.T, name string) *gnodrpc {
	datadir := initGnod(t)
	return startGnodWithIpc(t, name, "--datadir", datadir, "--nodiscover", "--syncmode=light", "--nat=extip:127.0.0.1", "--verbosity=4")
}
