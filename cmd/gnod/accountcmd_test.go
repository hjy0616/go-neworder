...
`
	...
	{
		gnod := runGnod(t, "account", "list", "--datadir", datadir)
		gnod.Expect(want)
		gnod.ExpectExit()
	}
	{
		gnod := runGnod(t, "--datadir", datadir, "account", "list")
		gnod.Expect(want)
		gnod.ExpectExit()
	}
}
...
func TestAccountNew(t *testing.T) {
	gnod := runGnod(t, "account", "new", "--lightkdf")
	defer gnod.ExpectExit()
	gnod.Expect(`
...
func TestAccountHelp(t *testing.T) {
	gnod := runGnod(t, "account", "-h")
	gnod.WaitExit()
	if have, want := gnod.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}

	gnod = runGnod(t, "account", "import", "-h")
	gnod.WaitExit()
	if have, want := gnod.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gnod := runGnod(t, "--lightkdf", "account", "import", "-password", passwordFile, keyfile)
	defer gnod.ExpectExit()
	gnod.Expect(expected)
}
...
func TestWalletImport(t *testing.T) {
	gnod := runGnod(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gnod.ExpectExit()
	gnod.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := os.ReadDir(filepath.Join(gnod.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	gnod := runMinimalGnod(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gnod.StderrText(), m) {
...
// https://github.com/neworder-network/go-neworder/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gnod.SetTemplateFunc("keypath", func(file string) string {
