...
import (
	...
	"github.com/neworder-network/go-neworder/accounts/external"
	"github.com/neworder-network/go-neworder/accounts/keystore"
	"github.com/neworder-network/go-neworder/accounts/scwallet"
	"github.com/neworder-network/go-neworder/accounts/usbwallet"
...
func loadConfig(file string, cfg *gnodConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.IPCPath = "gnod.ipc"
	return cfg
}

// makeConfigNode loads gnod configuration and creates a blank node instance.
func makeConfigNode(ctx *cli.Context) (*node.Node, gnodConfig) {
	// Load defaults.
	cfg := gnodConfig{
...
// makeFullNode loads gnod configuration and creates the Neworder backend.
func makeFullNode(ctx *cli.Context) (*node.Node, ethapi.Backend) {
	...
	if eth != nil && !ctx.IsSet(utils.IgnoreLegacyReceiptsFlag.Name) {
		...
		if err != nil {
			...
			stack.Close()
			utils.Fatalf("Database has receipts with a legacy format. Please run `gnod db freezer-migrate`.")
		}
	}
	...
	// Add the Neworder Stats daemon if requested.
	if cfg.Ethstats.URL != "" {
...
func applyMetricConfig(ctx *cli.Context, cfg *gnodConfig) {
