...
// nodeDockerfile is the Dockerfile required to run an Neworder node.
var nodeDockerfile = `
FROM neworder/client-go:latest
...
{{end}}
RUN \
  echo 'gnod --cache 512 init /genesis.json' > gnod.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.neworder/keystore/ && cp /signer.json /root/.neworder/keystore/' >> gnod.sh && \{{end}}
	echo $'exec gnod --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> gnod.sh

ENTRYPOINT ["/bin/sh", "gnod.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an Neworder node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.neworder{{if .Ethashdir}}
...
// deployNode deploys a new Neworder node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 gnod --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.neworder"],
