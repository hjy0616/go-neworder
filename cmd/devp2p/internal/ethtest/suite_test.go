...
import (
	...
	"github.com/neworder-network/go-neworder/internal/utesting"
...
func TestEthSuite(t *testing.T) {
	gnod, err := runGnod()
	if err != nil {
		t.Fatalf("could not run gnod: %v", err)
	}
	defer gnod.Close()

	suite, err := NewSuite(gnod.Server().Self(), fullchainFile, genesisFile)
	if err != nil {
...
// runGnod creates and starts a gnod node
func runGnod() (*node.Node, error) {
...
func setupGnod(stack *node.Node) error {
