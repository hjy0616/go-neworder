...
// Package les implements the Light Neworder Subprotocol.
package les
...
import (
	...
	"github.com/neworder-network/go-neworder/common/mclock"
	...
	"github.com/neworder-network/go-neworder/les/downloader"
	"github.com/neworder-network/go-neworder/les/vflux"
	vfc "github.com/neworder-network/go-neworder/les/vflux/client"
	"github.com/neworder-network/go-neworder/light"
	...
	"github.com/neworder-network/go-neworder/p2p/enr"
...
func New(stack *node.Node, config *ethconfig.Config) (*LightNeworder, error) {
...
func (s *LightNeworder) VfluxRequest(n *enode.Node, reqs vflux.Requests) vflux.Replies {
	if !s.udpEnabled {
		return nil
	}
...
func (s *LightNeworder) vfxVersion(n *enode.Node) uint {
	if n.Seq() == 0 {
		var err error
		if !s.udpEnabled {
			return 0
...
func (s *LightNeworder) prenegQuery(n *enode.Node) int {
	if s.vfxVersion(n) < 1 {
...
// APIs returns the collection of RPC services the neworder package offers.
// NOTE, some of these services probably need to be moved to somewhere else.
func (s *LightNeworder) APIs() []rpc.API {
...
func (s *LightNeworder) ResetWithGenesisBlock(gb *types.Block) {
	s.blockchain.ResetWithGenesisBlock(gb)
}

func (s *LightNeworder) BlockChain() *light.LightChain      { return s.blockchain }
func (s *LightNeworder) TxPool() *light.TxPool              { return s.txPool }
func (s *LightNeworder) Engine() consensus.Engine           { return s.engine }
func (s *LightNeworder) LesVersion() int                    { return int(ClientProtocolVersions[0]) }
func (s *LightNeworder) Downloader() *downloader.Downloader { return s.handler.downloader }
func (s *LightNeworder) EventMux() *event.TypeMux           { return s.eventMux }
func (s *LightNeworder) Merger() *consensus.Merger          { return s.merger }

// Protocols returns all the currently configured network protocols to start.
func (s *LightNeworder) Protocols() []p2p.Protocol {
...
// light neworder protocol implementation.
func (s *LightNeworder) Start() error {
...
// Neworder protocol.
func (s *LightNeworder) Stop() error {
	close(s.closeCh)
	s.serverPool.Stop()
	s.peers.close()
	s.reqDist.close()
	...
	s.lesDb.Close()
	s.wg.Wait()
	log.Info("Light neworder stopped")
	return nil
}
