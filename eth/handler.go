...
import (
	...
	"github.com/neworder-network/go-neworder/eth/fetcher"
...
// support all the operations needed by the Neworder chain protocols.
...
// newHandler returns a handler for all Neworder chain management protocol.
...
func (h *handler) runEthPeer(peer *eth.Peer, handler eth.Handler) error {
	...
	// Execute the Neworder handshake
	var (
	...
	if err := peer.Handshake(h.networkID, td, hash, genesis.Hash(), forkID, h.forkFilter); err != nil {
		peer.Log().Debug("Neworder handshake failed", "err", err)
		return err
	}
	...
	peer.Log().Debug("Neworder peer connected", "name", peer.Name())

	// Register the peer locally
	if err := h.peers.registerPeer(peer, snap); err != nil {
		peer.Log().Error("Neworder peer registration failed", "err", err)
		return err
	}
...
func (h *handler) unregisterPeer(id string) {
	...
	peer := h.peers.peer(id)
	if peer == nil {
		logger.Error("Neworder peer removal failed", "err", errPeerNotRegistered)
		return
	}
	// Remove the `eth` peer if it exists
	logger.Debug("Removing Neworder peer", "snap", peer.snapExt != nil)
	...
	if err := h.peers.unregisterPeer(id); err != nil {
		logger.Error("Neworder peer removal failed", "err", err)
	}
}
...
func (h *handler) Stop() {
	...
	log.Info("Neworder protocol stopped")
}
