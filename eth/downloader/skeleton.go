// Copyright 2022 The go-neworder Authors
...
// the process. This is usually the case when Gnod is shutting down and some events
...
//
// Opposed to the original Neworder block synchronization which is trustless (and
...
// not from within (vs. legacy Neworder sync).
//
...
func (s *skeleton) startup() {
	...
	for {
		...
		case errc := <-s.terminate:
			// No head was announced but Gnod is shutting down
			errc <- nil
			return
		...
		case event := <-s.headEvents:
			...
			for {
				...
				default:
					...
					// error. Abort and wait until Gnod requests a termination.
					errc := <-s.terminate
					errc <- err
					return
				}
