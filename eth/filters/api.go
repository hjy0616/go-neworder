...
// information related to the Neworder protocol such als blocks, transactions and logs.
...
func (api *FilterAPI) Logs(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	...
	logsSub, err := api.events.SubscribeLogs(neworder.FilterQuery(crit), matchedLogs)
	if err != nil {
		return nil, err
	}
...
// Same as neworder.FilterQuery but with UnmarshalJSON() method.
type FilterCriteria neworder.FilterQuery
...
func (api *FilterAPI) NewFilter(crit FilterCriteria) (rpc.ID, error) {
	...
	logsSub, err := api.events.SubscribeLogs(neworder.FilterQuery(crit), logs)
	if err != nil {
		return "", err
	}
