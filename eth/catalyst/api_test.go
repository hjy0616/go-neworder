...
var (
	...
	// testAddr is the Neworder address of the tester account.
...
func startEthService(t *testing.T, genesis *core.Genesis, blocks []*types.Block) (*node.Node, *eth.Neworder) {
	t.Helper()
...
func setupBlocks(t *testing.T, ethservice *eth.Neworder, n int, parent *types.Block, callback func(parent *types.Block)) {
	api := NewConsensusAPI(ethservice)
	for i := 0; i < n; i++ {
		callback(parent)
