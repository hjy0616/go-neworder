...
import (
	...
	"github.com/neworder-network/go-neworder/core/beacon"
...
type ethNode struct {
	...
	ethBackend *eth.Neworder
	lapi       *lescatalyst.ConsensusAPI
	lesBackend *les.LightNeworder
}
...
func makeFullNode(genesis *core.Genesis) (*node.Node, *eth.Neworder, *ethcatalyst.ConsensusAPI, error) {
	// Define the basic configurations for the Neworder node
	...
	// Create the node and configure a full Neworder node on it
...
func makeLightNode(genesis *core.Genesis) (*node.Node, *les.LightNeworder, *lescatalyst.ConsensusAPI, error) {
	// Define the basic configurations for the Neworder node
	datadir, _ := os.MkdirTemp("", "")
