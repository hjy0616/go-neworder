...
func (w *worker) commit(env *environment, interval func(), update bool, start time.Time) error {
	if w.isRunning() {
		...
		// https://github.com/neworder-network/go-neworder/issues/24299
		env := env.copy()
...
func totalFees(block *types.Block, receipts []*types.Receipt) *big.Float {
	...
	return new(big.Float).Quo(new(big.Float).SetInt(feesWei), new(big.Float).SetInt(big.NewInt(params.Nod)))
}
