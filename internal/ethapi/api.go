...
import (
	...
	"github.com/neworder-network/go-neworder/accounts/abi"
	"github.com/neworder-network/go-neworder/accounts/keystore"
	"github.com/neworder-network/go-neworder/accounts/scwallet"
	"github.com/neworder-network/go-neworder/common"
	"github.com/neworder-network/go-neworder/common/hexutil"
	"github.com/neworder-network/go-neworder/common/math"
	"github.com/neworder-network/go-neworder/consensus/ethash"
	"github.com/neworder-network/go-neworder/consensus/misc"
	...
	"github.com/neworder-network/go-neworder/crypto"
	"github.com/neworder-network/go-neworder/eth/tracers/logger"
...
// NeworderAPI provides an API to access Neworder related information.
type NeworderAPI struct {
	b Backend
}

// NewNeworderAPI creates a new Neworder protocol API.
func NewNeworderAPI(b Backend) *NeworderAPI {
	return &NeworderAPI{b}
}

// GasPrice returns a suggestion for a gas price for legacy transactions.
func (s *NeworderAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *NeworderAPI) MaxPriorityFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	tipcap, err := s.b.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *NeworderAPI) FeeHistory(ctx context.Context, blockCount rpc.DecimalOrHex, lastBlock rpc.BlockNumber, rewardPercentiles []float64) (*feeHistoryResult, error) {
...
func (s *NeworderAPI) Syncing() (interface{}, error) {
...
// NeworderAccountAPI provides an API to access accounts managed by this node.
// It offers only methods that can retrieve accounts.
type NeworderAccountAPI struct {
	am *accounts.Manager
}

// NewNeworderAccountAPI creates a new NeworderAccountAPI.
func NewNeworderAccountAPI(am *accounts.Manager) *NeworderAccountAPI {
	return &NeworderAccountAPI{am: am}
}

// Accounts returns the collection of accounts this node manages.
func (s *NeworderAccountAPI) Accounts() []common.Address {
...
// Sign calculates an Neworder ECDSA signature for:
// keccak256("\x19Neworder Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/neworder-network/go-neworder/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19Neworder Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/neworder-network/go-neworder/wiki/Management-APIs#personal_ecRecover
func (s *PersonalAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Neworder signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and gnod.
...
// BlockChainAPI provides an API to access Neworder blockchain data.
type BlockChainAPI struct {
	b Backend
}

// NewBlockChainAPI creates a new Neworder blockchain API.
func NewBlockChainAPI(b Backend) *BlockChainAPI {
	return &BlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the current Neworder chain config.
//
...
// See: https://github.com/neworder-network/wiki/wiki/JSON-RPC-Error-Codes-Improvement-Proposal
...
//
// https://github.com/neworder-network/wiki/wiki/JSON-RPC#eth_sign
...
// DebugAPI is the collection of Neworder APIs exposed over the debugging
// namespace.
...
// Version returns the current neworder protocol version.
...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	feeEth := new(big.Float).Quo(new(big.Float).SetInt(new(big.Int).Mul(gasPrice, new(big.Int).SetUint64(gas))), new(big.Float).SetInt(big.NewInt(params.Nod)))
	feeFloat, _ := feeEth.Float64()
	if feeFloat > cap {
		return fmt.Errorf("tx fee (%.2f nod) exceeds the configured cap (%.2f nod)", feeFloat, cap)
	}
	return nil
}
