...
// Package gnodclient provides an RPC client for gnod-specific APIs.
package gnodclient
...
// Client is a wrapper around rpc.Client that implements gnod-specific functionality.
//
// If you want to use the standardized Neworder RPC functionality, use ethclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
func (ec *Client) CreateAccessList(ctx context.Context, msg neworder.CallMsg) (*types.AccessList, uint64, string, error) {
...
func (ec *Client) CallContract(ctx context.Context, msg neworder.CallMsg, blockNumber *big.Int, overrides *map[common.Address]OverrideAccount) ([]byte, error) {
...
// GCStats retrieves the current garbage collection stats from a gnod node.
...
// MemStats retrieves the current memory stats from a gnod node.
...
// GetNodeInfo retrieves the node info of a gnod node.
