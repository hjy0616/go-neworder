...
// Verify that Client implements the neworder interfaces.
var (
	_ = neworder.ChainReader(&Client{})
	_ = neworder.TransactionReader(&Client{})
	_ = neworder.ChainStateReader(&Client{})
	_ = neworder.ChainSyncReader(&Client{})
	_ = neworder.ContractCaller(&Client{})
	_ = neworder.GasEstimator(&Client{})
	_ = neworder.GasPricer(&Client{})
	_ = neworder.LogFilterer(&Client{})
	_ = neworder.PendingStateReader(&Client{})
	// _ = neworder.PendingStateEventer(&Client{})
	_ = neworder.PendingContractCaller(&Client{})
)
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	// Create Neworder Service
	config := &ethconfig.Config{Genesis: genesis}
	config.Ethash.PowMode = ethash.ModeFake
	ethservice, err := eth.New(n, config)
	if err != nil {
		t.Fatalf("can't create new neworder service: %v", err)
	}
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == neworder.NotFound {
		t.Fatal("error should not be nil/notfound")
	}

	// Test tx in block not found.
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 20); err != neworder.NotFound {
		t.Fatal("error should be neworder.NotFound")
	}
}
