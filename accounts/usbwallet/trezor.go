...
import (
	...
	"github.com/neworder-network/go-neworder/accounts/usbwallet/trezor"
...
// or whether the Neworder app was not started on it.
...
// Neworder address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.NeworderAddress)
	if _, err := w.trezorExchange(&trezor.NeworderGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.NeworderTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.NeworderTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
