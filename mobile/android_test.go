...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gnod.LightScryptN, Gnod.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.neworder", new NodeConfig());
			node.start();
			...
			NeworderClient ec = node.getNeworderClient();
	...
	// signatures too. Regression test for go-neworder issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/neworder/gnodtest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gnod and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.neworder", "github.com/neworder-network/go-neworder/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "gnod.aar"), "gnod.aar")

	if err = os.WriteFile(filepath.Join("src", "androidTest", "java", "org", "neworder", "gnodtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
